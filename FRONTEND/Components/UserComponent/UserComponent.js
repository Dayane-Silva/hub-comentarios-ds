...
const loadComment = (onlyUserComments = false) => {
    ...
    CommentService.apiGetComment().then(result => {
        ...
        if (onlyUserComments) {
            // Filtrar apenas os comentários do usuário logado
...
const UserComponent = {
    run: () => {
        ...
        const btnMeusComentariosMDados = document.getElementById('btnMeusComentariosMDados');
        btnMeusComentariosMDados.addEventListener('click', function() {
            loadComment(true); 
        });
    }
}
